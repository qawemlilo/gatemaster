...
passport.use(new LocalStrategy({ usernameField: 'cell' }, function(cell, password, done) {
  User.findOne({cell: cell.trim().replace(/ /g, '')}, function(err, user) {
    if (!user) return done(null, false, { message: 'There is an account that is already using that number <' + cell + '>'});
    if (user.deletedAt) return done(null, false, { message: 'Your landlord has removed your number from our system'});
